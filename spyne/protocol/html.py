...
from spyne.util import six, coroutine
...
class HtmlBase(ProtocolBase):
    ...
    def subserialize(self, ctx, cls, inst, parent, ns=None, name=None):
        ...
        if cls.Attributes.max_occurs > 1:
            ...
            return self.serialize_array(ctx, cls, inst, parent, name, ctx.locale)
    ...
    def _get_members(self, ctx, cls, inst, parent, locale):
        for k, v in cls.get_flat_type_info(cls).items():
            ...
            if subvalue is not None and mo > 1:
                ...
                ret = self.serialize_array(ctx, v, subvalue, parent, sub_name, locale)
                if ret is not None:
                    while True:
                        sv2 = (yield)
                        ret.send(sv2)
    ...
    def subserialize(self, ctx, cls, inst, parent, ns=None, name=None):
        ...
        with parent.element('table', attrs):
            ...
            with parent.element('tbody'):
                if cls.Attributes.max_occurs > 1:
                    print self, "subser array", cls
                    ret = self.serialize_array(ctx, cls, inst, parent, name, ctx.locale)
...
class _HtmlColumnTable(_HtmlTableBase):
    ...
    def serialize_model_base(self, ctx, cls, inst, parent, name, locale):
        ...
        retval = E.td(self.to_string(cls, inst), **{self.field_name_attr: name})
        parent.write(retval)

    @coroutine
    def serialize_complex_model(self, ctx, cls, inst, parent, name, locale):
        attrs = {}
        print self, "ser com enter", cls
