...
__all__ = [
    'model_base_to_string', 'model_base_to_string_iterable', 'model_base_to_dict',
...
def decimal_from_string(cls, string):
    if cls.Attributes.max_str_len is not None and len(string) > cls.Attributes.max_str_len:
        raise ValidationError("Decimal '%s' longer than %d characters", [string, cls.Attributes.max_str_len])
...
def integer_from_string(cls, string):
    if cls.Attributes.max_str_len is not None and len(str(string)) > cls.Attributes.max_str_len:
        raise ValidationError("Integer '%s' longer than %d characters", [string, cls.Attributes.max_str_len])

    try:
        return int(string)
    except ValueError:
        raise ValidationError("Could not cast '%s' to integer", [string])
...
def time_from_string(cls, string):
    ...
    if match is None:
        raise ValidationError("time data '%s' does not match regex '%s'" %(string, _time_re.pattern))
